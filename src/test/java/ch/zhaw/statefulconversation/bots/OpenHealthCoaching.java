...
class OpenHealthCoaching {
  ...
  private static final String PROMPT_RAPPORTBUILDING = """
      ...
      Ask about their general well-being, hobbies, weekend plans—avoid politics or religion.
  ...
  void setUp() {
    ...
    State bestCaseFinal = new Final(OpenHealthCoaching.PROMPT_FINAL,
        OpenHealthCoaching.PROMPT_FINAL_STARTER);
    ...
    Transition fromRapportBuildingToCoaching = new Transition(
        List.of(new StaticDecision(OpenHealthCoaching.PROMPT_RAPPORTBUILDING_TRIGGER)),
        List.of(new StaticExtractionAction(
            OpenHealthCoaching.PROMPT_RAPPORTBUILDING_ACTION,
            storage, storageKeyToGoalDetected),
            new TransferUtterancesAction(coachingState)),
        coachingState);
    State rapportBuildingState = new State(
        OpenHealthCoaching.PROMPT_RAPPORTBUILDING,
        "RapportBuilding",
        OpenHealthCoaching.PROMPT_RAPPORTBUILDING_STARTER,
        List.of(fromRapportBuildingToCoaching));

    State finalFromOuter = new Final();
    Transition userExitTransition = new Transition(
        List.of(new StaticDecision(OpenHealthCoaching.PROMPT_OUTERSTATE_TRIGGER),
            new StaticDecision(OpenHealthCoaching.PROMPT_OUTERSTATE_GUARD)),
        List.of(new TransferUtterancesAction(finalFromOuter)),
        finalFromOuter);
    State outerState = new OuterState(
        OpenHealthCoaching.PROMPT_OUTERSTATE,
        "OuterState",
        List.of(userExitTransition),
        rapportBuildingState);

    Agent agent = new Agent("Wellness Navigator",
        "Wellness Navigator helps patients uncover what matters for their health and transform those insights into actionable self-care steps for everyday well-being.",
        outerState,
        storage);
    agent.start();
